...
const { isYouTubeUrl, cleanYouTubeUrl, resolveCookiesPath, downloadYouTube } = require('../scraper/youtube')
const { extractYouTubeUrl, handleYouTubeCase } = require('../case/youtube')
...
test('mengekstrak URL YouTube dari command', () => {
...
test('case YouTube mengirim video dan audio melalui mock WhatsApp', async () => {
  ...
  process.env.YTDLP_BIN = fixture
  try {
    const video = await handleYouTubeCase({ alip, m: { chat: '12345@s.whatsapp.net' }, text: 'https://youtu.be/abc123', mode: 'video' })
